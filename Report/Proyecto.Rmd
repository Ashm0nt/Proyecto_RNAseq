...
Dada la naturaleza del diseño del estudio (muestras pareadas de los mismos sujetos en diferentes condiciones) y la inherente heterogeneidad biológica de la población geriátrica, se implementó la siguiente estrategia bioinformática

- `variancePartition`: Cuantificar la contribución relativa de las variables experimentales (sujeto, tratamiento, visita) a la varianza global de los datos de expresión génica, lo que permitió confirmar que la identidad del Sujeto era la fuente dominante de variabilidad, estableciendo la necesidad de un modelo estadístico capaz de manejar efectos aleatorios.

- `dream`: Integrar la variabilidad interindividual como un efecto aleatorio dentro del marco de trabajo de limma/voom, permitiendo identificar genes diferencialmente expresados con mayor sensibilidad y robustez frente al ruido biológico, a pesar del tamaño de muestra limitado, permitiendo una estimación más precisa de los coeficientes de regresión para la variable de interés (tratamiento con metformina).

Además se utilizaron diferentes visualizaciones para explorar la calidad de los datos, la estructura de varianza y los resultados de expresión diferencial, incluyendo boxplots para la profundidad de secuenciación, histogramas para la proporción de lecturas asignadas a genes, MA plots y Volcano plots para la expresión diferencial, y heatmaps para patrones de expresión en genes seleccionados. También MDS plots  para evaluar la separación de las muestras en un espacio reducido, comparando la topología global del transcriptoma frente a la topología definida exclusivamente por los genes identificados mediante el modelo.
...
```{r transform_counts_and_metadata, message=FALSE, warning=FALSE}
# ------------------------------------------------------------
...
# Identificar sujetos incompletos automáticamente
...
kable(design_table,
      caption = "Distribución de muestras por sujeto y tratamiento")
```
...
```{r lectures_genes, message=FALSE, warning=FALSE, echo=TRUE, fig.align="center", fig.width=12, fig.height=8, out.width="0.9\\linewidth"}
...
# Plot de lecturas asignadas a genes por tratamiento
ggplot(
  df_qc,
...
kable(tab,
      caption = "Distribución de muestras por sujeto después del filtrado")
```
...
```{r filter_genes, message=FALSE, warning=FALSE, echo=TRUE}
# Obtener counts
...
```{r MDS, message=FALSE, warning=FALSE, echo=TRUE}
...
```{r variance_partition, message=FALSE, warning=FALSE, echo=TRUE}
...
```{r differential_expression, message=FALSE, warning=FALSE, echo=TRUE}
# Analisis de expresión diferencial con dream
...
### MA plot
```{r MA_plot, message=FALSE, warning=FALSE, echo=TRUE, fig.width=14,fig.height=10, out.width="\\linewidth"}
...
```{r Volcano_plot, message=FALSE, warning=FALSE, echo=TRUE, fig.width=16, fig.height=12, out.width="\\linewidth",fig.align='center'}
...
```{r count_genes_categories, message=FALSE, warning=FALSE, echo=TRUE}
# Guardar resultados 
...
### HEATMAP de genes seleccionados

```{r, results='asis', echo=FALSE}
cat("\\newpage")
```

```{r heatmap, message=FALSE, warning=FALSE, echo=TRUE, fig.width=16, fig.height=12, out.width="\\linewidth",fig.align='center'}
# ELegir top genes
n_top <- 25
...
```{r variance_top_genes_up, message=FALSE, warning=FALSE, echo=TRUE, fig.align="center", fig.width=12, fig.height=8, out.width="0.9\\linewidth"}
...
```{r variance_top_genes_down, message=FALSE, warning=FALSE, echo=TRUE, fig.align="center", fig.width=12, fig.height=8, out.width="0.9\\linewidth"}
# Crear objetos
plotPercentBars(vp_down_sorted) +
...
```{r MDS_top_genes, message=FALSE, warning=FALSE, echo=TRUE}
